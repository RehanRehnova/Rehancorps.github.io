...
def api_passfill_reviews():
    ...
    if not st["configured"]:
        ...
        if not st["spreadsheet_id"]:
            missing.append("SPREADSHEET_ID (create .env or spreadsheet_id.txt)")
    ...
    if not ok:
        # permission / API errors → 502 so client can show real message
        code = 400 if err and "rating" in err.lower() or (err and "short" in err.lower()) else 502
...
def api_sheets_status():
    """Debug: what config is detected (no secrets)."""
